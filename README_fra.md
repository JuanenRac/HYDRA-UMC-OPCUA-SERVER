...
**Vérification d'honnêteté - ce qui fonctionne réellement aujourd'hui :** le serveur OPC-UA et le mappage de l'espace d'adressage (`src/server.ts`) sont réels et testés (22 tests passants entre `tests/server.test.ts`, `tests/security.test.ts`, `tests/security-policy.test.ts`) - et ce sont de vrais tests au niveau du protocole : `tests/server.test.ts` connecte un vrai `OPCUAClient` (le propre client de node-opcua, la même bibliothèque qu'utiliseraient UAExpert/Ignition) via le vrai protocole binaire sur un vrai port TCP, et `tests/security-policy.test.ts` prouve qu'une session non chiffrée `SecurityPolicy.None` est authentiquement refusée par défaut (acceptée uniquement via l'échappatoire explicite `OPCUA_ALLOW_INSECURE=1`). Ce qui est délibérément différé, pas une lacune trouvée par accident : l'arbre de l'espace d'adressage est statique, construit une seule fois au démarrage - un robot ajouté plus tard nécessite un redémarrage pour apparaître, pas d'arbre dynamique par robot ; seules les lectures/écritures sont testées de bout en bout pour l'instant, pas les abonnements OPC-UA ; et le Pub/Sub (Phase 1 de la feuille de route) reste non implémenté, car aucun client n'en dépend encore. Voir `mejoras_futuras.txt` pour la liste complète de ce qui est différé et pourquoi, et `CHANGELOG.md` pour ce qui a été livré exactement jusqu'à présent.
...
* 🛡️ **Cryptage :** chaque vraie session exige Signed ou Signed+Encrypted (Basic256Sha256, Aes128_Sha256_RsaOaep ou Aes256_Sha256_RsaPss) - une connexion non chiffrée `SecurityPolicy.None` est réellement refusée, pas seulement déconseillée.
...
* **Ce qui se passe quand la source réelle de `SpindleTemp` se déconnecte.** La dernière vraie valeur continue d'être servie - jamais masquée, jamais réinitialisée - mais son `statusCode` passe de `Good` au code réel et standard de node-opcua `UncertainLastUsableValue` ("Whatever was updating this value has stopped doing so"), et `sourceTimestamp` continue de pointer vers l'instant réel de la dernière observation, jamais "maintenant". Une session correctement authentifiée et chiffrée ne change jamais cela - elle authentifie le client, pas la fraîcheur de la donnée.
...
Délibérément reporté, et pourquoi : un arbre d'espace d'adressage dynamique par robot (celui d'aujourd'hui est statique, construit une fois au démarrage - un robot ajouté plus tard nécessite un redémarrage pour apparaître) ; une couverture de test réelle pour les abonnements OPC-UA (seules les lectures/écritures sont testées de bout en bout pour l'instant) ; et le point Pub/Sub de la feuille de route (toujours non implémenté - aucun client n'en dépend pour l'instant).
...
├── mejoras_futuras.txt  # La liste complète et honnête de ce qui est délibérément reporté, et pourquoi
└── README.md
```
