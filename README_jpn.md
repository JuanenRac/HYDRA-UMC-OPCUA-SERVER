...
**正直な現状確認 - 今日実際に動くもの:** OPC-UA サーバーとアドレス空間のマッピング(`src/server.ts`)は本物であり、テスト済みである(`tests/server.test.ts`、`tests/security.test.ts`、`tests/security-policy.test.ts` 全体で 22 件のテストが成功)。しかもこれらは本物のプロトコルレベルのテストである: `tests/server.test.ts` は本物の `OPCUAClient`(node-opcua 自身のクライアントであり、UAExpert/Ignition が使うのと同じライブラリ)を、本物のバイナリプロトコルで本物の TCP ポートに接続しており、`tests/security-policy.test.ts` は暗号化されていない `SecurityPolicy.None` セッションがデフォルトで実際に拒否されることを証明している(明示的な `OPCUA_ALLOW_INSECURE=1` という抜け道を使った場合のみ受け入れられる)。以下は偶然見つかったギャップではなく、意図的に先送りされているものである: アドレス空間ツリーは静的であり、起動時に一度だけ構築される——後から追加されたロボットが表示されるには再起動が必要であり、ロボットごとの動的なツリーにはなっていない。今のところエンドツーエンドでテストされているのは読み書きのみであり、OPC-UA のサブスクリプションはテストされていない。そして Pub/Sub(ロードマップのフェーズ 1)はまだ実装されていない、依存するクライアントがまだ存在しないためである。何が先送りされ、なぜそうなのかの完全なリストは `mejoras_futuras.txt` を、これまでに何が実際に出荷されたかは `CHANGELOG.md` を参照。
...
* 🛡️ **暗号化：** すべての実際のセッションで Signed または Signed+Encrypted（Basic256Sha256、Aes128_Sha256_RsaOaep、または Aes256_Sha256_RsaPss）が必須です - 暗号化されていない `SecurityPolicy.None` 接続は単に非推奨なのではなく、実際に拒否されます。
...
* **`SpindleTemp` の実際のソースが切断されたときに起こること。** 最後の実際の値は引き続き提供され続けます——隠されることも、リセットされることもありません——しかしその `statusCode` は `Good` から node-opcua 自身の本物の標準 OPC-UA ステータスコードである `UncertainLastUsableValue`(「この値を更新していたものが更新を停止した」)に低下し、`sourceTimestamp` は「今」ではなく実際に最後に観測された瞬間を指し続けます。正しく認証・暗号化されたセッションであっても、これは変わりません——それが認証するのはクライアントであって、データの鮮度ではありません。
...
意図的に後回しにしたものとその理由:ロボットごとの動的なアドレス空間ツリー(現状は起動時に一度だけ構築される静的なツリーで、後から追加したロボットを表示するには再起動が必要)、OPC-UA サブスクリプションの実際のテストカバレッジ(現時点では読み書きのみがエンドツーエンドでテスト済み)、ロードマップの Pub/Sub 項目(まだ未実装 - 依存するクライアントはまだない)。
...
├── mejoras_futuras.txt  # 意図的に後回しにした内容とその理由を完全にまとめた正直なリスト
└── README.md
```
